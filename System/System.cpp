...
//////////////////////////////////////////////////
std::vector<double> EquationSystem::get_solution(){
    std::cout<<"Solving equation system...\n";
    Eigen::VectorXd x_zw = L_.fullPivLu().solve(b_);
...
void EquationSystem::generate_equation_system(){
    ...
    std::set<int> result;
    std::set_difference(all_numbers.begin(), all_numbers.end(), to_remove.begin(), to_remove.end(),std::inserter(result, result.end()));
    
    //reshape L so that columns corresponding to boundaries are neglected
    L_ = Eigen::MatrixXd::Zero(m_size_, result.size());
...
//generates koordinates corresponding to filled rectangle(x1<x2,y1<y2)
...
////////////////////////////////
void EquationSystem::write_to_csv(std::vector<double> x){
    std::ofstream file;
    file.open("../system.csv");
    file<<"x,y,value"<<"\n";
    for(int k=0;k<x.size();k++){
        int i = k%size_;
        int j = k/size_;
        file<<i*dx_<<","<<dx_*j<<","<<x[k]<<"\n";
    }
    file.close();
